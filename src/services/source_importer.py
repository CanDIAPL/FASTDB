...
class SourceImporter:
    ...
    def _read_mongo_fields( self, dbcon, collection, pipeline, fields,
        ...
        if not self.debug_just_read_mongo:
            ...
            # merged from carlo's branch into main
            # dbcon.execute( f"CREATE TEMP TABLE {temptable} (LIKE {liketable})" )
            ...
            if liketable == 'diaobject':
                dbcon.execute( f"ALTER TABLE {temptable} ALTER COLUMN rootid DROP NOT NULL" )
                dbcon.execute( f"ALTER TABLE {temptable} ALTER COLUMN spatial_id DROP NOT NULL" )
    ...
    def import_objects_from_collection( self, collection, t0=None, t1=None, batchsize=10000,
        ...
        with db.DBCon( dbcon ) as dbcon:
            ...
            dbcon.cursor.execute( "SELECT diaobjectid, ra, dec, validitystartmjdtai FROM temp_new_diaobject" )
            rows = dbcon.cursor.fetchall()

            for diaobjectid, ra, dec, mjd in rows:
                spatial_id = generate_spatial_id( ra, dec, mjd, self.procver, self.data_release )
                dbcon.execute( "UPDATE temp_new_diaobject SET spatial_id = %s WHERE diaobjectid = %s",
            ...
            # dbcon.execute( "INSERT INTO diaobject ( SELECT * FROM temp_new_diaobject ) "
    ...
    def import_sources( self, t0=None, t1=None, batchsize=10000, dbcon=None, commit=True ):
        ...
        with db.DBCon( dbcon ) as dbcon:
            ...
            dbcon.execute( "UPDATE temp_diasource_import tsi "
    ...
    def import_forcedsources( self, t0=None, t1=None, batchsize=10000, dbcon=None, commit=True ):
        ...
        with db.DBCon( dbcon ) as dbcon:
            # Populate spatial_id from diaobject (not available in MongoDB)
            dbcon.execute( "UPDATE temp_prvdiasource_import tsi "
                            "SET spatial_id = o.spatial_id "
                            "FROM diaobject o "
                            "WHERE o.diaobjectid = tsi.diaobjectid "
                            "  AND o.base_procver_id = tsi.base_procver_id" )
            dbcon.execute( "INSERT INTO diasource ( SELECT * FROM temp_prvdiasource_import ) ON CONFLICT DO NOTHING" )
    ...
    def import_brokerinfo( self, t0=None, t1=None, batchsize=10000, dbcon=None, commit=True ):
        with db.DBCon( dbcon ) as dbcon:
            ...
            # Populate spatial_id from diaobject (not available in MongoDB)
            dbcon.execute( "UPDATE temp_prvdiaforcedsource_import tsi "
                            "SET spatial_id = o.spatial_id "
                            "FROM diaobject o "
                            "WHERE o.diaobjectid = tsi.diaobjectid "
                            "  AND o.base_procver_id = tsi.base_procver_id" )
